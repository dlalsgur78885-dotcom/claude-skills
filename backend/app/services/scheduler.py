...
# 절대 삭제 금지 — 사용자 영구 자산 (저장된 템플릿 에셋, 브랜드 로고, 최종 산출물,
# CTA 업로드). 이 set 이 모든 정리 경로의 single source of truth. 신규 cleanup
# 로직 추가 시 반드시 이 set 을 참조해서 제외할 것.
...
# 3시간 ephemeral 정리에서 제외할 폴더. NEVER_DELETE + proxy_cache(7일 룰로 별도 관리).
PERMANENT_FOLDERS: set[str] = set(NEVER_DELETE_FOLDERS) | {"proxy_cache"}

# 재생성 가능한 장기 캐시 — 일주일 지나면 삭제. 다시 필요해지면 원본 재요청.
# restyled(AI 생성 이미지)는 카루셀 콘텐츠라 cutouts/enhanced 와 동일하게 보호한다.
# 예전엔 ephemeral(3h) 목록에 잘못 들어가 생성 이미지가 3시간 후 사라졌다.
LONG_LIVED_DAYS = 7
LONG_LIVED_FOLDERS: tuple[str, ...] = ("proxy_cache", "cutouts", "enhanced", "restyled")
